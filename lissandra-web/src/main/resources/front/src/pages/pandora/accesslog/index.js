...
class AccessLog extends PureComponent {
    ...
    get tableColumns() {
        return [{
            ...
            render: (text, record) => (
                <span>
            <a onClick={this.onShowModalClicked.bind(this, record.logId,record)}>查看更多</a>
          </span>
            )
        }]
    }
    ...
    onShowModalClicked(logId,record) {
        this.props.dispatch({
    ...
    setShowForm(data) {
        this.showForm.setFieldsValue(data, this.record);
    }
...
// 展示对话框 - 取消
    ...
    componentDidMount() {
        if (this.props.search) {
            this.loadTableDataSource({addition: this.props.search})
            return
        }
        TyHistory.auto()
        TyHistory.notSame(() => {
